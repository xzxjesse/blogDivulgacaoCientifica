<%@ Page Title="Sobre" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="About.aspx.cs" Inherits="WebForms_ConectaCiencia.About" %>

<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    <div class="container my-4">
        <div class="row">
            <div class="col-md-4">
                <h2>Objetivo</h2>
                <p>O Conecta Ciência tem como objetivo facilitar o acesso ao conhecimento científico, promovendo a disseminação de temas acadêmicos de forma acessível e interativa. Nosso propósito é criar um espaço para professores, estudantes e pesquisadores discutirem, explorarem e compartilharem ciência.</p>
            </div>
            <div class="col-md-4">
                <h2>Como Funciona</h2>
                <p>Aqui, você pode encontrar todo tipo de artigos organizados cronologicamente e por categoria. Além disso, você pode sugerir temas e artigos, contribuindo ativamente com a ciência.</p>
            </div>
            <div class="col-md-4">
                <h2>Contato</h2>
                <p>Quer falar com a gente? Entre em contato através de <a href="mailto:conectaciencia@gmail.com">conectaciencia@gmail.com</a> ou envie uma sugestão de tema ou artigo pelo formulário Sua Voz na Ciência.</p>
            </div>
        </div>
    </div>
</asp:Content>
